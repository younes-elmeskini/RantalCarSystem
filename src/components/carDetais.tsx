    ...
    <div className="flex flex-col lg:flex-row gap-8 lg:gap-[100px] justify-center items-start px-4 lg:px-0">
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
          {[
            { icon: "/icons/gear.png", label: "Gear Box", value: car.transmission },
            { icon: "/icons/fuel.png", label: "Fuel", value: car.fuelType },
            { icon: "/icons/dors.png", label: "Doors", value: car.dors },
            { icon: "/icons/air.png", label: "Air Conditioner", value: car.airConditioning ? "A/C" : "No A/C" },
            { icon: "/icons/seat.png", label: "Seats", value: car.seats, colSpan: "col-span-2 md:col-span-1" }
          ].map((spec, index) => (
            <motion.div 
              key={spec.label}
              className={`bg-[#fafafa] p-3 md:p-4 rounded-[20px] w-full md:w-[150px] flex-col gap-2 md:gap-4 flex justify-center items-start shadow-md text-sm md:text-[16px] hover:shadow-lg transition-shadow duration-300 ${spec.colSpan || ''}`}
